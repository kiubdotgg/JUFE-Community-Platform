...
$("#authForm").addEventListener("submit", async (e) => {
  ...
  if (!email || !password) {
    $("#authError").textContent = "请填写完整信息"; return;
  }

  if (isRegisterMode) {
    const username = $("#authUsername").value.trim();
    const confirm = $("#authConfirmPassword").value.trim();
    if (!username) { $("#authError").textContent = "请输入用户名"; return; }
    if (password !== confirm) { $("#authError").textContent = "两次密码不一致"; return; }
    if (password.length < 6) { $("#authError").textContent = "密码至少6位"; return; }

    const res = await api.register({ username, email, password });
    if (res.code !== 0) { $("#authError").textContent = res.msg; return; }
    toast("注册成功，请登录");
    toggleAuthMode();
  } else {
...
async function loadApp() {
  ...
  if (res.code !== 0) { localStorage.removeItem("token"); return; }
...
async function loadFeed() {
  ...
  if (res.code !== 0) { $("#feedList").innerHTML = '<div class="empty-state"><p>加载失败</p></div>'; return; }
...
function setupComposer() {
  ...
  $("#publishBtn").onclick = async () => {
    ...
    if (!content) { toast("请输入动态内容", "error"); return; }
...
function renderCommentTree(comments, postId, depth = 0) {
  if (!comments || !comments.length) return '<p style="font-size:13px;color:var(--text-secondary);padding:8px 0">暂无评论</p>';
...
function initReplyButtons() {
  setTimeout(() => {
    ...
    $$("[data-submit-reply]").forEach(btn => {
      btn.onclick = () => {
        const parentId = parseInt(btn.dataset.reply);
...
async function loadRecommend() {
  ...
  if (res.code !== 0) { $("#recommendList").innerHTML = '<div class="empty-state"><p>暂无推荐</p></div>'; return; }
